...
nes2016.clean <- function(interview = c("pre", "post", "both")) {
  ...
  nes2016_temp <- nes2016_raw %>%
    dplyr::mutate(
      Postvote.num = recode(voteResult, "HillaryClinton" = 0, "DonaldTrump" = 1),
  ...
  if (interview == "pre") {
    nes2016 <- nes2016_temp %>%
      ...
      dplyr::select(-c(
        agegroup, voteResult,
        ClintonLR, Postvote.num, WeightforPostElection
      )) %>% # Remove useless variables
      dplyr::select(c(
        age, edu.year, education, income.num, income,
    ...
    nes2016 <- nes2016_temp %>%
      ...
      dplyr::select(-c(
        education, agegroup, income, IntendVote,
        ClintonLR,
        Prevote.num, WeightforPreElection
      )) %>% # Remove useless variables
      dplyr::select(c(
        age, edu.year, income.num,
