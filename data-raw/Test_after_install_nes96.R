...
fit.vote <- glm(vote.num ~ income.num + age + edu.year,
  data = nes96,
...
PAsso_2 <- PAsso(
  responses = c("vote.num", "PID", "selfLR"),
  adjustments = c("income.num", "age", "edu.year"),
  data = nes96,
...
Pcor_SR_test1$corr
Pcor_SR_test1$corr_stat
Pcor_SR_test1$corr_p.value
Pcor_SR_test1$CI_95
...
Pcor_SR_test2$sd_MatCor
Pcor_SR_test2$corr
Pcor_SR_test2$corr_stat
...
dta <- dplyr::select(
  nes96, vote.num, PID, selfLR, ClinLR, DoleLR,
...
PAsso_5v <- PAsso(
  responses = c("vote.num", "PID", "selfLR", "ClinLR", "DoleLR"),
  adjustments = c("income.num", "age", "edu.year"),
  data = nes96,
  association = c("partial"),
  method = c("kendall"), resids.method = "latent", rep_num = 30
...
PAsso_5v_test1$sd_MatCor
PAsso_5v_test1$corr
PAsso_5v_test1$corr_p.value
print(PAsso_5v_test1, 5)

system.time(PAsso_5v_test <- test(object = PAsso_5v, boot_SE = 20, H0 = 0, parallel = FALSE))
PAsso_5v_test$sd_MatCor
PAsso_5v_test$corr
PAsso_5v_test$corr_stat
PAsso_5v_test$corr_p.value
PAsso_5v_test$CI_95
