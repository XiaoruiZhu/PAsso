...
fit.vote_test <- vglm(PreVote.num ~ income.num + age + edu.year,
  data = ANES2016,
...
fit.PID_test <- vglm(PID ~ income.num + age + edu.year,
  data = ANES2016,
  family = acat(reverse = TRUE, parallel = TRUE)
)
...
# Test jittering
system.time(PAsso_adv1_jit <- PAsso(
  fitted.models = list(fit.vote, fit.PID),
  association = c("partial"),
  method = c("kendall"),
  resids.type = "surrogate", jitter = "uniform",
